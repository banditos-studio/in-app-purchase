    ...
    function _onProductValidate(error, res, body) {
        ...
        if (body.purchaseState !== 0) {
            switch (body.purchaseState) {
                ...
                case 2:
                    verbose.log(NAME, 'Purchase is pending:', body);
                    return cb(new Error('PurchasePending'), createErrorData(receipt, {
                        status: constants.VALIDATION.FAILURE,
                        message: 'PurchasePending'
                    }));
                default:
                    verbose.log(NAME, 'Purchase state is unknown:', body);
                    return cb(new Error('PurchaseStateUnknown'), createErrorData(receipt, {
                        status: constants.VALIDATION.FAILURE,
                        message: 'PurchaseStateUnknown'
                    }));
            }
        }
